...
## 2025-05-27 - Input Accessibility Without Labels\n**Learning:** Relying solely on `placeholder` attributes for form inputs creates an accessibility anti-pattern because placeholders are not reliably read by all screen readers and disappear when the user starts typing, leaving them without context.\n**Action:** When a visible `<label>` is not feasible due to design constraints, always provide an `aria-label` attribute on the `<input>` element to ensure screen readers can announce the purpose of the field clearly.
