...
## 2024-04-18 - Heavy Synchronous Library Calls inside Forms Anti-Pattern
**Learning:** Calling heavy synchronous library functions (like `country-state-city`'s `Country.getAllCountries()`) directly inside a React component's render body causes expensive O(N) recalculations on every local state change, leading to severe input lag in forms.
**Action:** Always wrap static or derived data generation that depends on heavy operations in `useMemo` hooks to prevent unnecessary re-execution during form input typing or other local state updates.
