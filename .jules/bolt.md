...
## 2024-05-19 - [Product Admin List Optimization]
**Learning:** `Product.countDocuments()` is an O(N) operation when scanning the entire collection. Parallelizing query execution via `Promise.all` with `Product.estimatedDocumentCount()` reduces latency for endpoints returning unpaginated datasets like `getAdminProducts`.
**Action:** Always prefer `estimatedDocumentCount()` for total counts in pagination/list endpoints unless explicit filters require `countDocuments()`. Parallelize independent DB queries.
