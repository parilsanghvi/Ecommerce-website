...
## 2025-04-09 - Memoizing Synchronous Library Functions in React Components
**Learning:** `Shipping.jsx` was previously calculating lists of countries and states by calling `Country.getAllCountries()` and `State.getStatesOfCountry(country)` directly within the render body. These function calls execute synchronous O(N) operations. Because they were not memoized, they unnecessarily recalculated on every single keystroke when the user typed in inputs like address, city, phone number, and pin code.
**Action:** Always wrap expensive or synchronous data retrieval from libraries (like `country-state-city`) inside a `useMemo` hook with strict dependencies when calculating derived state in React components to prevent unnecessary re-evaluations during frequent state updates.
