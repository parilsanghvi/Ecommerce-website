...
## 2026-05-03 - Optimization of Product count for Admin Panel\n**Learning:** Using Mongoose `countDocuments()` triggers an O(N) full collection scan which is slow for large collections. `estimatedDocumentCount()` retrieves the count from collection metadata, achieving O(1) performance.\n**Action:** Use `estimatedDocumentCount()` when counting all documents without query filters. Also, beware of installing dependencies without explicit instructions as it alters lock files.
