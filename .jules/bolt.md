...
## 2025-03-10 - Order Stock Verification Optimization
**Learning:** During order status updates (e.g., to "Shipped"), checking product stock by mapping over items and using `Array.prototype.find()` on the fetched products array results in an O(N^2) operation. Furthermore, fetching the entire product document and hydrating it via Mongoose when only the `stock` field is needed wastes memory and processing time.
**Action:** When validating a specific field across a collection of documents (like stock for order items), chain `.select('stock')` and `.lean()` to the Mongoose query. Then, convert the resulting array into a JavaScript `Map` (keyed by the document ID) to reduce the lookup complexity inside the loop from O(N*M) to O(N+M). Ensure test mocks accurately reflect chained Mongoose methods (e.g., using `mockReturnValue({ select: jest.fn().mockReturnThis(), lean: jest.fn().mockResolvedValue(...) })`).
