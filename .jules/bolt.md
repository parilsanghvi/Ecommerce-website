...
## 2024-07-04 - Optimize findOne Query
**Learning:** For existences checks, append `.lean()` to Mongoose query.
**Action:** When only checking if a document exists, use `.lean()` on queries so that Mongoose returns plain JSON documents instead of hydrating full Mongoose documents, which decreases memory usage and speeds up responses.
