...
## 2024-04-11 - [Mongoose Optimization Map Lookups]
**Learning:** Nested array `.find()` calls against Mongoose results are an easy O(M*N) trap in node apps when iterating.
**Action:** When pre-verifying nested data models (like comparing order items to stock), convert the `find` result array to a `Map` keyed by ID for O(1) lookups, dropping the complexity to O(M+N). Always append `.select().lean()` when only checking specific properties.
