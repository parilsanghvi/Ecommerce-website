...
## 2025-02-28 - Insecure File Upload Parameter Parsing\n**Vulnerability:** The `/admin/product/new` and `/admin/product/:id` endpoints used `upload.any()` for parsing multipart/form-data. This is dangerous because it allows attackers to upload files under arbitrary field names, potentially leading to unbounded file uploads or processing of unexpected input.\n**Learning:** Relying on `upload.any()` provides zero enforcement on the structure or expected naming of uploaded files. Multer should be configured to explicitly expect the specific field names required by the application.\n**Prevention:** Always restrict file uploads using methods like `upload.single("fieldname")`, `upload.array("fieldname")`, or `upload.fields([{ name: "fieldname" }])` to strictly validate incoming file structures.
