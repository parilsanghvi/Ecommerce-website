...
## 2024-05-14 - Arbitrary File Upload via upload.any()
**Vulnerability:** Found  being used in  for admin product creation and update routes. This allows attackers to upload arbitrary files since it accepts files from any field over the wire.
**Learning:** This existed because the developer likely wanted a quick way to handle multipart form data without specifying the exact field name.
**Prevention:** Always use restrictive Multer middleware like  or  to explicitly define allowed file fields and prevent unexpected file uploads.
## 2024-05-14 - Arbitrary File Upload via upload.any()
**Vulnerability:** Found `upload.any()` being used in `backend/routes/productRoute.js` for admin product creation and update routes. This allows attackers to upload arbitrary files since it accepts files from any field over the wire.
**Learning:** This existed because the developer likely wanted a quick way to handle multipart form data without specifying the exact field name.
**Prevention:** Always use restrictive Multer middleware like `upload.array('images')` or `upload.single('avatar')` to explicitly define allowed file fields and prevent unexpected file uploads.
