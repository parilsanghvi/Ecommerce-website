...
## 2024-05-24 - [Arbitrary File Upload via Multer]
**Vulnerability:** The `admin/product/new` and `admin/product/:id` endpoints used Multer's `upload.any()` middleware, which allows arbitrary file fields to be uploaded and processed, opening the app to DoS attacks and potentially RCE depending on storage mechanisms.
**Learning:** Overly permissive Multer configurations are a common anti-pattern when developers want a quick fix for file uploads.
**Prevention:** Always restrict file uploads to specific, expected field names using `upload.single('field')` or `upload.array('field')` to minimize the attack surface.
