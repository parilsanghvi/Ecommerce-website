...
## 2026-04-28 - Insecure File Upload via Multer upload.any()
**Vulnerability:** The product creation and update routes in `backend/routes/productRoute.js` utilized the `upload.any()` middleware. This allowed attackers to upload arbitrary files with unexpected field names, potentially bypassing intended restrictions or causing resource exhaustion.
**Learning:** Using `upload.any()` creates a broad attack surface as it processes all incoming multipart file streams, regardless of the expected schema.
**Prevention:**
1. Always use restricted file upload methods like `upload.single('field')` or `upload.array('field')` with explicit, expected field names.
2. Ensure that only anticipated file inputs are processed by the server.
