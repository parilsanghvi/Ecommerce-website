...
## 2026-05-15 - Arbitrary File Upload via Multer upload.any()\n**Vulnerability:** The `upload.any()` Multer middleware was used on product creation and update routes, which accepts files from any field name.\n**Learning:** This can lead to arbitrary file uploads or denial of service if an attacker sends unexpected files outside of the expected `images` field.\n**Prevention:** Always use restrictive Multer middleware like `upload.array('images')` or `upload.single('avatar')` to explicitly define which fields are allowed to upload files.
