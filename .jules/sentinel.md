...
## 2026-05-11 - Insecure File Upload via Multer upload.any()\n**Vulnerability:** The `upload.any()` Multer middleware was used in product creation and update routes. This is highly insecure as it allows processing of arbitrary file fields, potentially leading to arbitrary file uploads or disk space exhaustion attacks.\n**Learning:** Always explicitly restrict file uploads to the expected fields using `upload.single()` or `upload.array()` (e.g., `upload.array('images')`) to minimize the attack surface. Additionally, always revert `pnpm-lock.yaml` and `package.json` changes caused by running `pnpm install` during local testing unless explicitly instructed to update dependencies.\n**Prevention:**\n1. Never use `upload.any()` in production routes.\n2. Enforce strict field matching for multipart form data parsing.\n3. Always run `git status` and revert unintentional lockfile modifications before creating a PR.
