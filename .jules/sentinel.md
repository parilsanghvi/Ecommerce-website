...
## 2026-05-10 - Insecure Multer Middleware (upload.any())
**Vulnerability:** The `backend/routes/productRoute.js` used `upload.any()` middleware, which allows processing of arbitrary file fields.
**Learning:** This is insecure because it exposes the application to processing any file type.
**Prevention:** Always prefer restrictive methods like `upload.single()` or `upload.array('images')` with explicit field names to minimize attack surface.
