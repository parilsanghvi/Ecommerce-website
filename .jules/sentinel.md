...
## 2024-06-19 - Restrict Multer File Uploads
**Vulnerability:** Using `upload.any()` middleware allows processing of arbitrary file fields, increasing the attack surface.
**Learning:** The product routes used `upload.any()` when they only needed to accept an `images` field array.
**Prevention:** Always use restrictive methods like `upload.single()` or `upload.array()` with explicit field names to minimize the attack surface.
