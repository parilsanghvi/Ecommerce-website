...
## 2025-02-27 - Insecure Multer Upload Configuration
**Vulnerability:** The product creation and update routes used `upload.any()`, allowing attackers to upload arbitrary file fields to memory, potentially increasing the attack surface or leading to unexpected field parsing behavior.
**Learning:** `upload.any()` is a wildcard that defeats Multer's built-in field filtering.
**Prevention:**
1. Always use strict field name binding like `upload.array('images')` or `upload.single('avatar')` to reject unexpected file payloads at the middleware level.
