...
## 2025-02-27 - Multer `upload.any()` Arbitrary File Processing Vulnerability
**Vulnerability:** The application used `upload.any()` in the `productRoute.js` for endpoints accepting multipart form data. This allowed an attacker to upload arbitrary files under unexpected field names, potentially bypassing specific file validation logic or exhausting server resources if the arbitrary files aren't cleaned up properly by the subsequent processing logic.
**Learning:** `upload.any()` is overly permissive and dangerous in production applications, particularly when only specific file fields (like `images`) are expected.
**Prevention:**
1. Always use restrictive Multer configurations like `upload.single('field')` or `upload.array('field')` to explicitly define the expected file structure and field names.
2. Only use `upload.any()` or `upload.none()` when strictly necessary, and with robust validation.
