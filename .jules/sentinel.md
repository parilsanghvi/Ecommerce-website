...
## 2025-02-28 - Insecure File Upload Parameter Parsing
**Vulnerability:** The product creation and update routes used `upload.any()` middleware from Multer. This configuration instructs Multer to process any file field included in the request, regardless of name. This exposes the server to arbitrary file parsing and potential Denial of Service (DoS) attacks, as it would accept and process unexpected or excessive file uploads that the application logic does not handle.
**Learning:** Using `upload.any()` in an Express application significantly increases the attack surface for file upload endpoints. It's crucial to be explicit about which file fields are expected.
**Prevention:**
1. Never use `upload.any()` unless explicitly required and paired with extremely stringent downstream validation.
2. Always prefer restrictive middleware configurations like `upload.single('field')` or `upload.array('field', maxCount)`.
3. Explicitly define expected field names to ensure Multer only processes intended file uploads.
