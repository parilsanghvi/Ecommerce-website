...
## 2024-05-18 - Prevent Arbitrary File Uploads (DoS/RCE risk)
**Vulnerability:** The product creation and update routes in `productRoute.js` used `upload.any()`, allowing attackers to upload arbitrary file fields which could lead to DoS or potentially RCE.
**Learning:** `multer`'s `upload.any()` should never be used as it accepts all files over the wire without restrictions.
**Prevention:** Always use specific field restrictors like `upload.single('field')` or `upload.array('field')` to explicitly define allowed file inputs.
