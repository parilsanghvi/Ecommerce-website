...
## 2024-05-24 - Arbitrary File Upload via upload.any()
**Vulnerability:** The `/admin/product/new` and `/admin/product/:id` endpoints used `upload.any()` from Multer, allowing users to upload files with any field name, not just the expected `images` field.
**Learning:** Using `upload.any()` is overly permissive and increases the attack surface, potentially leading to DoS attacks by filling up the server's memory or unexpected behaviors if malicious payloads are crafted with unforeseen field names.
**Prevention:** Always restrict file uploads to the specific expected field names using `upload.single('fieldname')`, `upload.array('fieldname')`, or `upload.fields([{ name: 'fieldname', maxCount: 1 }])`.
