...
## 2025-05-18 - Prevent 500 Error Information Disclosure
**Vulnerability:** The error handling middleware (`backend/middleware/error.js`) was reflecting raw application errors and system details directly to the client on 500 Internal Server Errors, regardless of the environment.
**Learning:** Default unhandled exceptions and structural errors often contain sensitive context (like database connection strings, stack traces, or internal IP configurations) which attackers can use to map the backend architecture.
**Prevention:** Always implement an environment check (`NODE_ENV === 'production'`) in global error handlers. If an error is a 500 in production, mask it with a generic message like "internal server error" while continuing to log the real error server-side for observability.
