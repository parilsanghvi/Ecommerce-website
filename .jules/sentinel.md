...
## 2025-02-28 - Arbitrary File Upload via upload.any()
**Vulnerability:** The `upload.any()` Multer middleware allows processing of arbitrary file fields from multipart requests. This drastically increases the attack surface for DoS vulnerabilities, unhandled fields, and malicious files.
**Learning:** Multer's `any()` processes every file field indiscriminately. Using explicit field names prevents unwanted files from bypassing validation.
**Prevention:**
1. Always prefer restrictive file upload configurations such as `upload.single('field_name')` or `upload.array('field_name')`.
2. Explicity specify allowed field names so that extraneous files are ignored or rejected by multer.
