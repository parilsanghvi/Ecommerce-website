...
## 2024-05-30 - Form Input Accessibility & Loading States
**Learning:** Many form components within the user account flow (like ForgotPassword, UpdateProfile, UpdatePassword, and ResetPassword) lack explicit `<label>` elements for their inputs, relying entirely on visual placeholders or icons. This presents an accessibility barrier. Furthermore, these inputs were not universally disabling during asynchronous operations (`loading` state), meaning users could theoretically alter values mid-submission.
**Action:** When working on form inputs that lack explicit visual `<label>` elements, always ensure `aria-label` is applied to communicate the input's purpose to screen readers. Additionally, universally apply `disabled={loading}` to all interactive form inputs and their corresponding inline action buttons (like password toggles), not just the primary submit button, to prevent unexpected mid-submission state changes.
