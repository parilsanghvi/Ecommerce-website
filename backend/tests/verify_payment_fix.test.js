...
describe("Payment Controller Security Fix", () => {
  ...
  it("should calculate price on server and ignore client amount", async () => {
    ...
    Product.find.mockReturnValue({ select: jest.fn().mockReturnValue({ lean: jest.fn().mockResolvedValue(mockProducts) }) });
  ...
  it("should return error if product not found", async () => {
    const mockProducts = [{ _id: "prod1", price: 500, toString: () => "prod1" }];
    Product.find.mockReturnValue({ select: jest.fn().mockReturnValue({ lean: jest.fn().mockResolvedValue(mockProducts) }) }); // Only prod1 found
